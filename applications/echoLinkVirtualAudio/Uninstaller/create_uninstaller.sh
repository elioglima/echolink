...
file="/Library/Audio/Plug-Ins/HAL/EchoLinkVirtualAudio'$channels'ch.driver"
...
if [ -d "$file" ] ; then
	...
	# Build .pkg
    packageName='Uninstaller/EchoLinkVirtualAudio'$channels'ch-Uninstaller.pkg'

    pkgbuild --nopayload --scripts Uninstaller/Scripts --sign $devTeamID --identifier 'audio.neocoode.EchoLinkVirtualAudio'$channels'ch.Uninstaller' $packageName
